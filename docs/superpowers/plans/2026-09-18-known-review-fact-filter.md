...
- [ ] **Step 1: Add the registry with the official source URL**
...
- [ ] **Step 2: Write failing tests for exact suppression behavior**
...
denial, preservation of non-finding result fields, and a result with no
`findings` array. Do not use a broad substring assertion that would suppress
every message containing `ubuntu-slim`.

- [ ] **Step 3: Run the focused test and confirm it fails for the missing implementation**
...
- [ ] **Step 1: Load the registry relative to the module**
...
- [ ] **Step 2: Implement narrow claim matching**

Extract text from a finding's `message`, `body`, or `content` string fields.
Only match a finding when it contains a registered label as a whole token and
one of these existence/validity denial forms:
...
Do not match generic `unsupported`, timeout, cost, permission, container, or
availability claims. Those can be legitimate even when the label exists.

- [ ] **Step 3: Return filtered results without mutating caller input**
...
`Suppressed known false-positive rule(s): github-actions-runner-label:ubuntu-slim`
...
Expected: PASS for every suppression, preservation, malformed-input, and
non-mutating-result test.
...
- [ ] **Step 1: Add a dedicated Actions step**
...
- [ ] **Step 2: Verify no prompt or runner setting changed**
...
- [ ] **Step 3: Run the complete workflow-script test suite**
...
- [ ] **Step 1: Test the CLI against a temporary result file**
