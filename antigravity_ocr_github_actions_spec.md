...
flowchart TD
    ...
    subgraph Runner["GitHub Actions Runner (Ubuntu / Trusted Base)"]
        ...
        subgraph HostAgent["Antigravity Host Agent (agy -p)"]
            ...
            AGY -->|1. 対象ファイル抽出| Preview["ocr delegate preview --format json --rule ../self-repo/.github/workflows/config/markdown-review-rules.json --from origin/<base_ref> --to <commit_sha>"]
...
sequenceDiagram
    ...
    alt 外部フォーク PR
        ...
        AGY->>OCR: `ocr delegate preview --format json --rule ../self-repo/.github/workflows/config/markdown-review-rules.json --from origin/<base_ref> --to <commit_sha>`
  ...
  review:
    ...
    steps:
      ...
      - name: Install pinned review tools
        if: steps.target.outputs.internal == 'true'
        run: |
          npm install -g --ignore-scripts @alibaba-group/open-code-review@1.12.2
